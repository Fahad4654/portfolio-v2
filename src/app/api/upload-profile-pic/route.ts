...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (uploadError) {
      ...
      return NextResponse.json({ 
        message: `Image upload failed due to a server authentication error. Please ensure SUPABASE_SERVICE_ROLE_KEY and NEXT_PUBLIC_SUPABASE_URL are correctly set in your .env file and that the server has been restarted. Supabase error: ${uploadError.message}`
      }, { status: 500 });
    }
